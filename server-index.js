...
// Get pending Till payment count (public — used for badge)
app.get("/api/subscription/pending-count", async (_req, res) => {
  try {
    const db = getDb();
...
// Admin: list pending Till payments
...
// Admin: approve Till payment and issue license
app.post("/api/admin/till-approve", authMiddleware, (req, res) => {
  try {
    ...
    res.json({ success: true, message: "Payment approved! License issued.", expiresAt: end });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// Subscribe endpoint: also fires notification on M-Pesa submission
